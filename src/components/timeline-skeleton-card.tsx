...
export const ShimmerBar = ({
  ...
  const gradientColors = isDark ? ['transparent', 'rgba(255,255,255,0.12)', 'transparent'] : ['transparent', 'rgba(255,255,255,0.6)', 'transparent'];
  const shimmerStyle = [styles.shimmer, {
    width: shimmerWidth,
    transform: [{
      translateX
    }]
  }];
  return <View className={`relative overflow-hidden ${className ?? ''}`} style={style} onLayout={event => setWidth(event.nativeEvent.layout.width)}>
      {isActive && width > 0 ? <Animated.View pointerEvents="none" style={shimmerStyle}>
          <LinearGradient colors={gradientColors} start={{
        x: 0,
        y: 0
      }} end={{
        x: 1,
...
const TimelineSkeletonCard = ({
  ...
  return <DropShadowBox>
      <View className="relative bg-surface border-2 border-foreground dark:border-border px-5 py-4 overflow-hidden">
        {message ? <Text className="text-[14px] text-muted">{message}</Text> : <View className="flex-row gap-3">
            ...
            <View className="flex-1 gap-2">
              ...
              {Array.from({
            length: lineCount
          }).map((_, index) => <ShimmerBar key={`line-${index}`} className="h-3 w-full bg-surface-secondary" style={{
