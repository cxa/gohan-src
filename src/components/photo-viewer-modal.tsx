...
import Animated, { Easing, Extrapolation, interpolate, useAnimatedStyle, useSharedValue, withDelay, withDecay, withSpring, withTiming } from 'react-native-reanimated';
...
const PhotoViewerModal = ({
  ...
  }, [backdropOpacity, clearLoadingFallback, isDismissing, originRect, photoUrl, presentationScale, presentationTranslateX, presentationTranslateY, contentOpacity, originPreviewOpacity, scale, startScale, startTranslateX, startTranslateY, translateX, translateY, viewportHeight, viewportWidth, visible]);
  ...
  const startCloseTransition = () => {
    ...
    const endTranslateX = originExists ? originX.value + originWidth.value / 2 - viewportWidth / 2 : 0;
    const endTranslateY = originExists ? originY.value + originHeight.value / 2 - viewportHeight / 2 : 0;
    const endScale = originExists ? clamp(Math.min(originWidth.value / Math.max(baseWidth.value, 1), originHeight.value / Math.max(baseHeight.value, 1)), 0.08, 1) : 1;
    ...
    originPreviewOpacity.value = originExists && !originBehindTabBar ? withTiming(1, CLOSE_ORIGIN_FADE_IN_TIMING) : withTiming(0, CLOSE_ORIGIN_FADE_IN_TIMING);
    backdropOpacity.value = withDelay(140, withTiming(0, CLOSE_BACKDROP_FADE_OUT_TIMING, finished => {
      if (finished) {
  ...
  const pinchGesture = (() => {
    return Gesture.Pinch().onBegin(() => {
      ...
      const nextTranslateX = startTranslateX.value * ratio + (1 - ratio) * focalX;
      const nextTranslateY = startTranslateY.value * ratio + (1 - ratio) * focalY;
      const bounds = getPanBounds(nextScale);
      scale.value = nextScale;
      translateX.value = applyEdgeResistance(nextTranslateX, bounds.minX, bounds.maxX);
      translateY.value = applyEdgeResistance(nextTranslateY, bounds.minY, bounds.maxY);
    }).onEnd(() => {
  ...
  const panGesture = (() => {
    return Gesture.Pan().averageTouches(true).activeOffsetY([-4, 4]).minDistance(1).onBegin(() => {
      startTranslateX.value = translateX.value;
      startTranslateY.value = translateY.value;
    }).onUpdate(event => {
      if (isDismissing.value) {
      ...
      const verticalDrag = event.translationY > 0 ? event.translationY : event.translationY * 0.18;
      translateX.value = event.translationX * 0.16;
      translateY.value = verticalDrag;
      const dragDistance = Math.abs(verticalDrag);
      backdropOpacity.value = interpolate(dragDistance, [0, 220], [1, 0.3], Extrapolation.CLAMP);
    }).onEnd(event => {
      if (isDismissing.value) {
        return;
      }
      ...
      if (isZoomed) {
        const bounds = getPanBounds(scale.value);
        const clampX: [number, number] = [bounds.minX - EDGE_OVERSCROLL, bounds.maxX + EDGE_OVERSCROLL];
        const clampY: [number, number] = [bounds.minY - EDGE_OVERSCROLL, bounds.maxY + EDGE_OVERSCROLL];
        translateX.value = withDecay({
          ...
          translateX.value = withSpring(clamp(translateX.value, currentBounds.minX, currentBounds.maxX), PAN_SPRING);
        });
        translateY.value = withDecay({
          ...
          const currentBounds = getPanBounds(scale.value);
          translateY.value = withSpring(clamp(translateY.value, currentBounds.minY, currentBounds.maxY), PAN_SPRING);
        });
        return;
      }
      const shouldClose = translateY.value > DISMISS_DISTANCE || event.velocityY > DISMISS_VELOCITY || translateY.value > 56 && event.velocityY > 420;
      if (shouldClose) {
  ...
  const doubleTapGesture = (() => {
    return Gesture.Tap().numberOfTaps(2).maxDelay(240).maxDistance(18).onEnd(event => {
      if (isDismissing.value) {
        return;
      }
      const isZoomed = scale.value > MIN_SCALE + 0.01;
  ...
  const imageTransformStyle = useAnimatedStyle(() => {
    const dismissScale = scale.value <= MIN_SCALE + 0.01 ? interpolate(Math.abs(translateY.value), [0, 280], [1, 0.84], Extrapolation.CLAMP) : 1;
    return {
      transform: [{
  ...
  return <Modal visible={visible} transparent animationType="none" statusBarTranslucent onRequestClose={closeWithSharedTransition}>
          ...
          <View style={StyleSheet.absoluteFill}>
            ...
            <View style={styles.imageCenter} pointerEvents="box-none">
              <Animated.View style={presentationStyle}>
                ...
                }]} resizeMode="contain" onLoad={markImageLoaded} onError={markImageLoaded} onLoadEnd={markImageLoaded} />
                </Animated.View>
              </Animated.View>
            </View>
            {originRect ? <Animated.View pointerEvents="none" style={[styles.originPreview, {
            left: originRect.x,
            top: originRect.y,
            ...
            height: originRect.height
                ...
                <Image source={{
              uri: photoUrl
            }} style={styles.originPreviewImage} resizeMode="cover" />
              </Animated.View> : null}
          </View>
        </GestureDetector>
        {isImageLoading ? <View style={styles.loading} pointerEvents="none">
            <NeobrutalActivityIndicator size="small" color="#FFFFFF" />
          </View> : null}
        <Animated.View style={[styles.closeWrap, {
        top: Math.max(topInset + 8, 16)
      }, closeControlStyle]}>
          <Pressable onPress={closeWithSharedTransition} style={styles.closeButton} accessibilityRole="button" accessibilityLabel={t('photoViewerCloseA11y')}>
