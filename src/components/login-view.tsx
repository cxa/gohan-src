...
import Animated, { useAnimatedProps, useSharedValue, withRepeat, withTiming, Easing, FadeInDown } from 'react-native-reanimated';
...
const TechnicalBackground = ({
  ...
  return <View className="absolute inset-0 bg-[#F2F2F2] dark:bg-[#0A0A0A]" pointerEvents="none">
      <Svg width="100%" height="100%" style={StyleSheet.absoluteFill}>
        {/* Diagonal Cross */}
        <Line x1={TL_X} y1={TL_Y} x2={BR_X} y2={BR_Y} stroke={gridColor} strokeWidth="1" />
        <Line x1={TR_X} y1={TR_Y} x2={BL_X} y2={BL_Y} stroke={gridColor} strokeWidth="1" />

        {/* Center Crosshairs */}
        <Line x1={SCREEN_WIDTH / 2} y1="0" x2={SCREEN_WIDTH / 2} y2="15" stroke={crosshairColor} strokeWidth="1" />
        <Line x1={SCREEN_WIDTH / 2} y1={SCREEN_HEIGHT - 15} x2={SCREEN_WIDTH / 2} y2={SCREEN_HEIGHT} stroke={crosshairColor} strokeWidth="1" />
        <Line x1="0" y1={SCREEN_HEIGHT / 2} x2="15" y2={SCREEN_HEIGHT / 2} stroke={crosshairColor} strokeWidth="1" />
        <Line x1={SCREEN_WIDTH - 15} y1={SCREEN_HEIGHT / 2} x2={SCREEN_WIDTH} y2={SCREEN_HEIGHT / 2} stroke={crosshairColor} strokeWidth="1" />

        {/* Center Circle Guide */}
        <Circle cx={SCREEN_WIDTH / 2} cy={SCREEN_HEIGHT / 2 - 40} r="100" stroke={gridColor} strokeWidth="1" fill="none" />

        {/* Corner Crop Marks */}
        {/* Top Left */}
        <Path d={`M ${TL_X} ${TL_Y} L ${TL_X + 15} ${TL_Y} M ${TL_X} ${TL_Y} L ${TL_X} ${TL_Y + 15}`} stroke={cropMarkColor} strokeWidth="1" fill="none" />
        {/* Top Right */}
        <Path d={`M ${TR_X} ${TR_Y} L ${TR_X - 15} ${TR_Y} M ${TR_X} ${TR_Y} L ${TR_X} ${TR_Y + 15}`} stroke={cropMarkColor} strokeWidth="1" fill="none" />
        {/* Bottom Left */}
        <Path d={`M ${BL_X} ${BL_Y} L ${BL_X + 15} ${BL_Y} M ${BL_X} ${BL_Y} L ${BL_X} ${BL_Y - 15}`} stroke={cropMarkColor} strokeWidth="1" fill="none" />
        {/* Bottom Right */}
        <Path d={`M ${BR_X} ${BR_Y} L ${BR_X - 15} ${BR_Y} M ${BR_X} ${BR_Y} L ${BR_X} ${BR_Y - 15}`} stroke={cropMarkColor} strokeWidth="1" fill="none" />
      </Svg>
    </View>;
};
...
for (let i = 0; i <= LATS; i++) {
  ...
  for (let j = 0; j < LONS; j++) {
    ...
    const p1: [number, number, number] = [rLat * Math.cos(lon), yLat, rLat * Math.sin(lon)];
    const p2: [number, number, number] = [rLat * Math.cos(nextLon), yLat, rLat * Math.sin(nextLon)];
    const p3: [number, number, number] = [rNextLat * Math.cos(lon), yNextLat, rNextLat * Math.sin(lon)];
...
const useGlobePathProps = ({
  ...
  for (let i = 0; i < GEOMETRY.length; i++) {
    ...
    const lineJitter = Math.sin(sy1 * SIZE * 0.1 + t * 20) * 15 * glitchTrigger;
...
const TechnicalAnimation = () => {
  ...
  useEffect(() => {
    time.value = withRepeat(withTiming(Math.PI * 2, {
      duration: 15000,
  ...
  const cyanProps = useGlobePathProps({
    colorType: 'cyan',
    time,
    cx,
    cy
  ...
  const mainProps = useGlobePathProps({
    colorType: 'main',
    time,
    cx,
    cy
  });
  return <View className="absolute inset-0" pointerEvents="none">
      <Svg width="100%" height="100%" strokeLinecap="round" strokeLinejoin="round">
        {/* Chromatic aberration layers */}
        <AnimatedPath animatedProps={cyanProps} stroke="#00FFFF" strokeWidth="1.5" opacity={0.8} />
        <AnimatedPath animatedProps={magentaProps} stroke="#FF00FF" strokeWidth="1.5" opacity={0.8} />
        {/* Main geometry layer */}
        <AnimatedPath animatedProps={mainProps} stroke={mainColor} strokeWidth="1.5" />
      </Svg>
    </View>;
};
const LoginView = () => {
  ...
  return <View className="flex-1 bg-[#F2F2F2] dark:bg-[#0A0A0A]" style={containerStyle}>
