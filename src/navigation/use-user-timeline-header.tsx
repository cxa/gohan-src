...
const getUserById = (userId: string) => get('/users/show', {
  id: userId
...
const useUserTimelineHeader = ({
  ...
  const semanticTitle = screenTitle ? `${displayName} · ${screenTitle}` : displayName;
  const handleName = `@${user?.id || userId}`;
  const rightSubtitle = screenTitle?.trim() || null;
  const avatarUrl = user?.profile_image_url_large || user?.profile_image_url || null;
  const avatarInitial = displayName.slice(0, 1).toUpperCase() || '?';
  const backTitle = typeof backCount === 'number' ? new Intl.NumberFormat().format(backCount) : undefined;
  const headerTitle = () => <UserTimelineHeaderTitle displayName={displayName} handleName={handleName} avatarUrl={avatarUrl} avatarInitial={avatarInitial} />;
  const headerRightItems = rightSubtitle ? () => [{
    type: 'custom' as const,
    element: <View pointerEvents="none">
                  <Text className="text-right text-[14px] leading-4" style={{
        color: foreground,
  ...
  useLayoutEffect(() => {
    const targetNavigation = useParentNavigation ? navigation.getParent<NativeStackNavigationProp<ParamListBase>>() || navigation : navigation;
    if (!userId) {
  ...
  }, [backTitle, headerRightItems, headerTitle, navigation, screenTitle, semanticTitle, useParentNavigation, userId]);
};
