...
const MentionsRoute = () => {
  ...
  const errorMessage = error ? error instanceof Error ? error.message : t('mentionsLoadFailed') : null;
  ...
  return <>
      <ScrollShadow LinearGradientComponent={LinearGradient} size={100} color={background}>
        <FlatList ref={listRef} className="bg-background" data={timelineItems} keyExtractor={item => getStatusId(item)} refreshControl={refreshControl} onScroll={scrollHandler} scrollEventThrottle={timelineListSettings.scrollEventThrottle} scrollIndicatorInsets={timelineListSettings.scrollIndicatorInsets} contentContainerStyle={timelineListSettings.contentContainerStyle} ListFooterComponent={isFetchingMore ? <View className="items-center py-6">
              ...
              </View> : null} ListHeaderComponent={<TimelineTitleHeader title={t('mentionsTitle')} titleContainerStyle={titleContainerStyle} titleTextStyle={titleTextStyle} errorMessage={errorMessage} />} ListEmptyComponent={isLoading || isHydratingTimelineItems ? <TimelineSkeletonList keyPrefix="mentions-skeleton" availableHeight={skeletonAvailableHeight} /> : <TimelineSkeletonCard message={t('mentionsEmpty')} />} onEndReached={fetchMore} onEndReachedThreshold={0.4} renderItem={({
        item
