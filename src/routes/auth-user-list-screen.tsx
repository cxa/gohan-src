...
import { Image, Platform, Pressable, RefreshControl, useWindowDimensions, View } from 'react-native';
...
const getUsersByMode = (mode: 'following' | 'followers', userId: string, page: number): Promise<FanfouUser[]> => get(mode === 'following' ? '/users/friends' : '/users/followers', {
  id: userId,
...
const UserListRoute = () => {
  ...
  const route = useRoute<RouteProp<AuthStackParamList, typeof AUTH_STACK_ROUTE.USER_LIST>>();
  const {
    userId,
    mode,
    backCount
  ...
  const skeletonAvailableHeight = windowHeight - insets.top - PAGE_BOTTOM_PADDING - insets.bottom;
  ...
  const {
    ...
    error,
    refetch
  } = useInfiniteQuery<FanfouUser[], Error, InfiniteData<FanfouUser[]>, [string, 'following' | 'followers', string], number>({
    queryKey: ['user-list', mode, userId],
    queryFn: ({
      pageParam
    }) => getUsersByMode(mode, userId, pageParam),
    initialPageParam: 1,
    getNextPageParam: (lastPage, _allPages, lastPageParam) => lastPage.length === USERS_PAGE_SIZE ? lastPageParam + 1 : undefined,
    retry: 1
  });
  const items = (data?.pages ?? []).flatMap(pageItems => pageItems);
  const errorMessage = error ? getErrorMessage(error, t('userListLoadFailed')) : null;
  ...
  const refreshControl = <RefreshControl refreshing={isPullRefreshing} onRefresh={handlePullRefresh} tintColor="transparent" colors={['transparent']} />;
  return <>
      <NativeEdgeScrollShadow className="flex-1" color={background}>
        <Animated.FlatList className="flex-1 bg-background" data={items} keyExtractor={item => item.id} onScroll={scrollHandler} contentInsetAdjustmentBehavior="automatic" scrollIndicatorInsets={{
        bottom: insets.bottom
      }} contentContainerStyle={contentContainerStyle} onEndReachedThreshold={0.3} onEndReached={() => {
        if (!hasNextPage || isFetchingNextPage || isPending) {
          return;
        }
        fetchNextPage().catch(() => undefined);
      }} refreshControl={refreshControl} ItemSeparatorComponent={UserItemSeparator} ListHeaderComponent={errorMessage ? <View className="mb-4">
        ...
        const avatarUrl = item.profile_image_url_large || item.profile_image_url;
        const description = parseHtmlToText(item.description).trim();
        const initial = displayName.slice(0, 1).toUpperCase();
        return <DropShadowBox>
                <Pressable onPress={() => handleOpenProfile(item.id)} accessibilityRole="button" className="flex-row gap-3 border-2 border-foreground bg-surface px-4 py-4 dark:border-border active:translate-x-[-4px] active:translate-y-[4px]">
                  {avatarUrl ? <Image source={{
              uri: avatarUrl
            }} className="rounded-full bg-surface-secondary" style={{
              width: AVATAR_SIZE,
              height: AVATAR_SIZE
            }} /> : <View className="items-center justify-center rounded-full bg-surface-secondary" style={{
              width: AVATAR_SIZE,
              height: AVATAR_SIZE
            }}>
                  ...
                  <View className="flex-1">
                    ...
                    {description ? <Text className="mt-1 text-[13px] text-muted" numberOfLines={2}>
                        {description}
                      </Text> : null}
                  </View>
                </Pressable>
              </DropShadowBox>;
      }} ListEmptyComponent={isPending ? <View className="gap-6">
                {Array.from({
          length: countSkeletonItemsForHeight(skeletonAvailableHeight, USER_SKELETON_ITEM_HEIGHT, USER_SKELETON_GAP, USER_SKELETON_FALLBACK_COUNT)
        }).map((_, i) => <UserSkeletonCard key={i} />)}
              </View> : <DropShadowBox>
              ...
              </DropShadowBox>} ListFooterComponent={isFetchingNextPage ? <View className="items-center py-6">
