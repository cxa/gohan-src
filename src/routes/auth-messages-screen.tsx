...
import { Alert, FlatList, Image, Platform, Pressable, RefreshControl, useWindowDimensions, View } from 'react-native';
...
const MAILBOX_META: Record<MailboxKind, {
  label: string;
  emptyMessage: string;
  addressLabel: string;
...
const PostageStamp = ({
  ...
  height: STAMP_SIZE
    ...
    <View className="absolute overflow-hidden" style={{
    ...
    height: STAMP_CONTENT_SIZE
      ...
      uri: avatarUrl
    }} className="h-full w-full" /> : <View className="h-full w-full items-center justify-center bg-surface-secondary">
          <Text className="text-[10px] font-semibold text-muted">
            {initial}
          </Text>
        </View>}
    </View>
    <Svg width={STAMP_SIZE} height={STAMP_SIZE} viewBox="0 0 15 15" className="absolute inset-0">
      <Path d={POSTAGE_STAMP_PATH} fill="none" stroke={borderColor} strokeWidth="1" strokeLinecap="round" strokeLinejoin="round" />
    </Svg>
    <Svg width={STAMP_WAVE_SVG_WIDTH} height={STAMP_WAVE_SVG_HEIGHT} viewBox="0 0 18 19" style={STAMP_WAVE_SVG_STYLE}>
      {STAMP_WAVE_LINES.map(({
      d,
      opacity
    }, i) => <Path key={i} d={d} fill="none" stroke="#E63946" strokeWidth="0.6" strokeLinecap="round" opacity={opacity} />)}
    </Svg>
  </View>;
...
const MessageSkeletonCard = ({
  ...
  return <DropShadowBox containerClassName="w-full">
...
const MailboxHeaderTabs = ({
  ...
  return <View className="flex-row gap-4">
      ...
      return <View key={key} className="min-w-[96px]">
            <DropShadowBox shadowOffsetClassName="-translate-x-0.5 translate-y-0.5">
              <Pressable onPress={() => onPressTab(key)} className={`w-full border border-foreground dark:border-border px-2 py-1 ${isActive ? 'bg-accent' : 'bg-surface active:translate-x-[-3px] active:translate-y-[3px]'}`} accessibilityRole="button" accessibilityState={isActive ? {
            selected: true
          } : undefined} accessibilityLabel={key === 'inbox' ? t('messagesInbox') : t('messagesOutbox')}>
                <View className="flex-row items-center justify-center gap-1.5">
                  <Icon color={isActive ? activeIconColor : muted} size={12} />
                  <Text className={`text-[11px] ${isActive ? 'text-accent-foreground' : 'text-foreground'}`}>
...
const MessageCard = ({
  ...
  const counterpartId = counterpart?.id || (mailbox === 'inbox' ? message.sender_id : message.recipient_id);
  const displayName = counterpart?.screen_name || counterpart?.name || counterpartId || 'Unknown';
  ...
  const handleDelete = () => {
    ...
    Alert.alert(t('messageDeleteTitle'), t('messageDeleteConfirm', {
      name: displayName
    }), [{
  ...
  return <DropShadowBox containerClassName="w-full">
    ...
    }} disabled={!isPressable} accessibilityRole={isPressable ? 'button' : undefined} className={`relative w-full overflow-hidden bg-white dark:bg-surface border-2 border-foreground dark:border-border px-4 pb-4 pt-5 ${isPressable ? 'active:translate-x-[-4px] active:translate-y-[4px]' : ''}`}>
        {onReply ? <Pressable onPress={onReply} hitSlop={8} accessibilityRole="button" accessibilityLabel="Reply to message" className="absolute right-3 top-3 z-10 active:opacity-50">
            <Reply size={14} color={muted} />
          </Pressable> : null}
        <View className="flex-row gap-3">
          <PostageStamp avatarUrl={avatarUrl} initial={initial} borderColor={stampBorderColor} />
          ...
          <View className="relative flex-1">
            ...
            <View className="pl-3">
              <View className="min-w-0 flex-row items-baseline gap-1">
                <Text className="shrink text-[16px] font-semibold text-foreground" numberOfLines={1}>
                  {displayName}
                </Text>
                {handle ? <Text className="shrink-0 text-[12px] text-muted">
                    {handle}
                  </Text> : null}
              </View>

              {messageText ? <Text className="mt-2 text-[14px] leading-6 text-foreground">
                  {messageText}
                </Text> : null}
              ...
              <View className="mt-2 flex-row items-center justify-between">
                ...
                {onDelete ? <Pressable onPress={handleDelete} hitSlop={8} accessibilityRole="button" accessibilityLabel="Delete message" className="active:opacity-50">
...
const PrivateMessagesContent = ({
  ...
  const skeletonAvailableHeight = windowHeight - insets.top - NAV_HEADER_HEIGHT - insets.bottom - PAGE_BOTTOM_PADDING;
  const [background, muted, accentForeground, border] = useThemeColor(['background', 'muted', 'accent-foreground', 'border']);
  ...
  const {
    ...
    queryFn: async () => {
      const [inboxData, sentData] = await Promise.all([get('/direct_messages/inbox', {
        count: 40
  ...
  const renderHeaderTabs = () => <MailboxHeaderTabs activeMailbox={activeMailbox} muted={muted} activeIconColor={accentForeground} onPressTab={setActiveMailbox} />;
  useLayoutEffect(() => {
    if (!parentNavigation) {
      return undefined;
    }
  ...
  const errorMessage = error ? getErrorMessage(error, t('messagesLoadFailed')) : null;
  ...
  const handleDeleteMessage = async (messageId: string) => {
    try {
      ...
      queryClient.setQueryData<MessageGroups>(['private-messages', userId], prev => {
        if (!prev) {
          return prev;
        }
        return {
      ...
      Alert.alert(t('messageDeleteFailed'), getErrorMessage(deleteError, t('messageDeleteFailedMessage')));
    }
  };
  const handleSubmitReply = async ({
    ...
    try {
      ...
      Alert.alert(t('messageReplyFailed'), getErrorMessage(replyError, t('messageReplyFailedMessage')));
    }
  };
  ...
  const {
    isPullRefreshing,
    handlePullRefresh
  } = usePullRefreshState(refetch);
  const createRefreshControl = () => <RefreshControl refreshing={isPullRefreshing} onRefresh={handlePullRefresh} tintColor="transparent" colors={['transparent']} />;
  const renderMailboxList = (mailbox: MailboxKind, mailboxItems: FanfouDirectMessage[], listRef: React.RefObject<FlatList<FanfouDirectMessage> | null>) => <>
      <NativeEdgeScrollShadow className="flex-1" color={background}>
        <Animated.FlatList ref={listRef} className="flex-1 bg-background" data={mailboxItems} keyExtractor={item => `${mailbox}-${item.id}`} renderItem={({
        item
      }) => {
        const sender = item.sender;
        const senderId = sender?.id || item.sender_id;
        return <MessageCard message={item} mailbox={mailbox} onPressProfile={handleOpenProfile} onDelete={() => handleDeleteMessage(item.id)} onReply={mailbox === 'inbox' && senderId ? () => setReplyTarget({
          userId: senderId,
          displayName: sender?.screen_name || sender?.name || senderId
        }) : undefined} stampBorderColor={border} />;
      }} onScroll={scrollHandler} contentInsetAdjustmentBehavior="automatic" scrollIndicatorInsets={{
        bottom: insets.bottom
      }} contentContainerStyle={contentContainerStyle} refreshControl={createRefreshControl()} ListHeaderComponent={errorMessage ? <Surface className="bg-danger-soft px-4 py-3">
                <Text className="text-[13px] text-danger-foreground">
                  {errorMessage}
                </Text>
              </Surface> : null} ListHeaderComponentStyle={errorMessage ? {
        marginBottom: CARD_GAP
      } : undefined} ItemSeparatorComponent={MessageItemSeparator} ListEmptyComponent={isLoading ? <View className="gap-7">
                {Array.from({
          length: countSkeletonItemsForHeight(skeletonAvailableHeight, MESSAGE_SKELETON_ITEM_HEIGHT, MESSAGE_SKELETON_GAP, MESSAGE_SKELETON_FALLBACK_COUNT)
        }).map((_, i) => <MessageSkeletonCard key={i} shimmerIndex={i % 3} />)}
              </View> : <TimelineEmptyMessage message={mailbox === 'inbox' ? t('messagesInboxEmpty') : t('messagesOutboxEmpty')} />} />
  ...
  return <View className="flex-1 bg-background">
      ...
      <ComposerModal visible={replyTarget !== null} title={replyTarget ? t('messageReplyComposerTitle', {
      name: replyTarget.displayName
    }) : ''} placeholder={t('messageReplyPlaceholder')} submitLabel={t('messageSend')} topInset={insets.top} resetKey={replyTarget ? `dm-reply:${replyTarget.userId}` : ''} onCancel={() => setReplyTarget(null)} onSubmit={handleSubmitReply} />
    </View>;
};
