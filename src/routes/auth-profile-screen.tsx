...
import { Alert, Image, Platform, Pressable, RefreshControl, View } from 'react-native';
...
import ProfileStatRow, { type ProfileStatItem } from '@/components/profile-stat-row';
...
import type { AuthProfileStackParamList, AuthStackParamList } from '@/navigation/types';
...
const ProfileRouteContent = ({
  routeUserId
}: ProfileRouteContentProps) => {
  const {
    t
  } = useTranslation();
  ...
  const userQueryKey = ['profile', 'user', routeUserId] as const;
  ...
  const normalizedCurrentUserScreenName = currentUserScreenName ? currentUserScreenName.toLowerCase() : null;
  const isSelf = Boolean(currentUserId) && currentUserId === authUserId || Boolean(normalizedCurrentUserScreenName) && normalizedCurrentUserScreenName === normalizedAuthScreenName || normalizedRouteUserId === normalizedAuthScreenName;
  const {
  ...
  const isProtectedTimeline = Boolean(user && user.protected && !isSelf && user.following !== true);
  const recentStatusesQueryKey = ['profile', 'recent-statuses', routeUserId, currentUserId] as const;
  const {
    data: recentStatuses,
  ...
  const updateUserCache = (patch: Partial<FanfouUser>) => {
    queryClient.setQueryData<FanfouUser | null>(userQueryKey, previous => previous ? {
      ...previous,
      ...patch
    } : previous);
  };
  ...
  const handleFollowToggle = async () => {
    ...
    try {
      await post(nextFollowing ? '/friendships/create' : '/friendships/destroy', {
        id: user.id
      });
      updateUserCache({
      ...
      Alert.alert(t('successTitle'), nextFollowing ? t('profileFollowSuccess', {
        name
      }) : t('profileUnfollowSuccess', {
        name
      }));
    } catch (requestError) {
      Alert.alert(t('operationFailed'), getErrorMessage(requestError, t('profileFollowFailed')));
    } finally {
  ...
  const handleBlockToggle = async () => {
    ...
    try {
      ...
      const name = user.screen_name ?? user.name ?? '';
      Alert.alert(t('successTitle'), nextBlocked ? t('profileBlockSuccess', {
        name
      }) : t('profileUnblockSuccess', {
        name
      }));
    } catch (requestError) {
      Alert.alert(t('operationFailed'), getErrorMessage(requestError, t('profileBlockFailed')));
    } finally {
  ...
  const handleSendComposer = async ({
    ...
    try {
      ...
      setComposeReplyTarget(null);
      setComposeRepostTarget(null);
      Alert.alert(t('sentTitle'), composeMode === 'mention' ? t('profileMentionSent') : composeMode === 'dm' ? t('profileMessageSent') : composeMode === 'reply' ? t('replySent') : t('repostSent'));
    } catch (requestError) {
      Alert.alert(composeMode === 'repost' ? t('repostFailedTitle') : composeMode === 'reply' ? t('replyFailedTitle') : t('profileSendFailed'), getErrorMessage(requestError, t('profileSendFailedMessage')));
    }
  };
  const handleToggleBookmark = async (status: FanfouStatus) => {
    ...
    setRecentStatusItems(previous => previous.map(item => getStatusId(item) === statusId ? {
      ...item,
    ...
    try {
      ...
      queryClient.setQueryData<FanfouStatus[]>(recentStatusesQueryKey, previous => previous ? previous.map(item => getStatusId(item) === statusId ? {
        ...item,
        favorited: nextFavorited
      } : item) : previous);
    } catch (requestError) {
  ...
  const profileErrorMessage = error ? getErrorMessage(error, t('profileLoadFailed')) : null;
  const recentStatusesErrorMessage = recentStatusesError ? getErrorMessage(recentStatusesError, t('recentActivityEmpty')) : null;
  ...
  if (isLoading && !user) {
    return <View className="flex-1 bg-background">
        <Animated.ScrollView className="flex-1 bg-background" contentInsetAdjustmentBehavior="automatic" contentContainerStyle={contentContainerStyle} scrollEnabled={false}>
          <DropShadowBox containerClassName="mb-4">
            <ProfileSummaryCard avatar={<View className="h-20 w-20 rounded-full bg-surface-secondary" />} displayName="" skeleton />
          </DropShadowBox>
  ...
  const profileAvatar = hasAvatar ? <View className={photoViewerVisible && photoViewerPreviewKey === 'profile-avatar' ? 'opacity-0' : undefined}>
      <Pressable onPress={() => handlePhotoPress(avatarUrl, 'profile-avatar')} accessibilityRole="button" accessibilityLabel="Open avatar">
        <View ref={node => registerPhotoPreviewRef('profile-avatar', node)} collapsable={false} className="h-20 w-20">
          <Image source={{
          uri: avatarUrl
        }} className="h-full w-full rounded-full bg-surface-secondary" />
        </View>
      </Pressable>
    </View> : <View className="h-20 w-20 items-center justify-center rounded-full bg-surface-secondary">
  ...
  const composerTitle = composeMode === 'dm' ? t('profileMessageComposerTitle', {
    handle: handleName
  }) : composeMode === 'reply' ? composeReplyTarget ? t('composerReplyTo', {
  ...
  }) : t('composerRepost') : t('profileMentionComposerTitle', {
    handle: handleName
  });
  const composerPlaceholder = composeMode === 'dm' ? t('profileMessagePlaceholder') : composeMode === 'reply' ? t('composerReplyPlaceholder') : composeMode === 'repost' ? t('composerCommentPlaceholder') : t('composerReplyPlaceholder');
  const composerSubmitLabel = composeMode === 'dm' ? t('messageSend') : composeMode === 'reply' ? t('composerSubmitReply') : composeMode === 'repost' ? t('composerSubmitRepost') : t('composerSubmitPost');
  const composerInitialText = composeMode === 'mention' ? `@${user.screen_name} ` : composeMode === 'reply' && composeReplyTarget ? `@${composeReplyTarget.screenName} ` : '';
  const composerResetKey = composeMode === 'mention' ? `mention:${user.id}` : composeMode === 'dm' ? `dm:${user.id}` : composeMode === 'reply' ? `reply:${composeReplyTarget?.statusId ?? ''}` : composeMode === 'repost' ? `repost:${composeRepostTarget?.statusId ?? ''}` : 'closed';
  ...
  if (!accessToken) {
    return <View className="flex-1 bg-background px-6 pt-8">
        <Surface className="bg-danger-soft px-4 py-3">
          <Text className="text-[13px] text-danger-foreground">
            {t('notLoggedIn')}
          </Text>
  ...
  return <>
      <NativeEdgeScrollShadow className="flex-1 bg-background" color={background}>
        <Animated.ScrollView className="flex-1 bg-background" contentInsetAdjustmentBehavior="automatic" contentContainerStyle={contentContainerStyle} onScroll={scrollHandler} scrollEventThrottle={16} refreshControl={<RefreshControl refreshing={isPullRefreshing} onRefresh={handlePullRefresh} tintColor="transparent" colors={['transparent']} />}>
          <DropShadowBox containerClassName="mb-4">
            <ProfileSummaryCard avatar={profileAvatar} displayName={displayName} handleName={handleName} location={user.location} joinedAt={joinedAt} profileUrl={profileUrl} description={description} />
          </DropShadowBox>

          <ProfileStatRow stats={statsPrimary} />
          <ProfileStatRow stats={statsSecondary} />

          {!isSelf ? <DropShadowBox containerClassName="pb-2">
              <Surface className="bg-surface-secondary border-2 border-foreground dark:border-border px-4 py-4">
                <View className="flex-row gap-3">
                  <Pressable onPress={handleFollowToggle} disabled={isFollowSubmitting || isBlocked} className={`flex-1 border border-border px-3 py-2 ${isBlocked ? 'bg-surface' : 'bg-accent'}`} accessibilityRole="button" accessibilityLabel={isFollowing ? t('profileActionUnfollow') : t('profileActionFollow')}>
                    <Text className={`text-[13px] text-center ${isBlocked ? 'text-muted' : 'text-accent-foreground'}`}>
                      {isFollowSubmitting ? t('profileActionUpdating') : isBlocked ? t('profileActionBlock') : isFollowing ? t('profileActionUnfollow') : t('profileActionFollow')}
                    </Text>
                  </Pressable>

                  <Pressable onPress={handleBlockToggle} disabled={isBlockSubmitting || isBlockChecking} className="flex-1 border border-border bg-surface px-3 py-2" accessibilityRole="button" accessibilityLabel={isBlocked ? t('profileActionUnblock') : t('profileActionBlock')}>
                    <Text className="text-[13px] text-center text-foreground">
                      {isBlockChecking ? t('profileActionChecking') : isBlockSubmitting ? t('profileActionUpdating') : isBlocked ? t('profileActionUnblock') : t('profileActionBlock')}
                    </Text>
                  </Pressable>
                </View>

                <View className="mt-3 flex-row gap-3">
                  <Pressable onPress={handleOpenMentionComposer} className="flex-1 border border-border bg-surface px-3 py-2" accessibilityRole="button" accessibilityLabel={t('profileActionMention')}>
                    <Text className="text-[13px] text-center text-foreground">
                      {t('profileActionMention')}
                    </Text>
                  </Pressable>

                  <Pressable onPress={handleOpenDmComposer} className="flex-1 border border-border bg-surface px-3 py-2" accessibilityRole="button" accessibilityLabel={t('profileActionMessage')}>
                    <Text className="text-[13px] text-center text-foreground">
          ...
          {profileErrorMessage ? <Surface className="bg-danger-soft px-4 py-3">
              <Text className="text-[13px] text-danger-foreground">
                {profileErrorMessage}
              </Text>
            </Surface> : null}

          {isProtectedTimeline ? <DropShadowBox type="warning" containerClassName="pb-2">
              <Surface className={`bg-surface border-2 ${getDropShadowBorderClass('warning')} px-4 py-4`}>
                <Text className="text-[14px] leading-6 text-warning">
                  {t('protectedAccountNotice')}
                </Text>
              </Surface>
            </DropShadowBox> : <>
              <Text className="mt-4 text-[24px] font-semibold text-foreground" dynamicTypeRamp="title1">
                {t('recentActivity')}
              </Text>
              <View style={{
            gap: TIMELINE_SPACING
          }}>
                {isRecentStatusesLoading || isHydratingRecentStatuses ? <TimelineSkeletonList keyPrefix="profile-status-skeleton" count={3} className="" style={{
              gap: TIMELINE_SPACING
            }} /> : null}

                {recentStatusesErrorMessage ? <Surface className="bg-danger-soft px-4 py-3">
                    <Text className="text-[13px] text-danger-foreground">
                      {recentStatusesErrorMessage}
                    </Text>
                  </Surface> : null}

                {!isRecentStatusesLoading && !isHydratingRecentStatuses && recentStatusItems.length === 0 && !recentStatusesErrorMessage ? <TimelineSkeletonCard message={t('recentActivityEmpty')} /> : null}

                {recentStatusItems.map(status => <TimelineStatusCard key={getStatusId(status)} status={status} accent={accent} muted={muted} showAvatar={false} showAuthor={false} isBookmarkPending={pendingBookmarkIds.has(getStatusId(status))} photoViewerVisible={photoViewerVisible} photoViewerPreviewKey={photoViewerPreviewKey} registerPhotoPreviewRef={registerPhotoPreviewRef} onOpenPhoto={handlePhotoPress} onPressStatus={handleStatusPress} onPressProfile={handleMentionPress} onPressMention={handleMentionPress} onPressTag={handleTagPress} onReply={handleOpenReplyComposer} onRepost={handleOpenRepostComposer} onToggleBookmark={handleToggleBookmark} />)}
              </View>
            </>}

          {isLoading ? <View className="items-center py-3">
      ...
      <ComposerModal visible={composeMode !== null} title={composerTitle} placeholder={composerPlaceholder} submitLabel={composerSubmitLabel} topInset={insets.top} initialText={composerInitialText} resetKey={composerResetKey} enablePhoto={composeMode === 'mention' || composeMode === 'reply'} onCancel={handleCloseComposer} onSubmit={handleSendComposer} />
    </>;
};
const ProfileRoute = () => {
  const route = useRoute<RouteProp<AuthProfileStackParamList, typeof AUTH_PROFILE_ROUTE.DETAIL>>();
